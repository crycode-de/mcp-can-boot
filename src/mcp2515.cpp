 ...
 *
 * Originally taken from https://github.com/autowp/arduino-mcp2515
 *
 * Modified by Peter Müller <peter@crycode.de> (https://crycode.de) for
 * MCP-CAN-Boot to use less flash space and work correctly within the
 * bootloader section.
 */
...
void MCP2515::init () {
  ...
  // setup spi
  #if F_CPU < 4000000L
    // < 4MHz ... SCK frequency = oscillator frequency / 4
    SPCR = ((1<<SPE) | (1<<MSTR) | (1<<CPOL) | (1<<CPHA));
  #elif F_CPU < 8000000L
    // < 8MHz ... SCK frequency = oscillator frequency / 8
    SPCR = ((1<<SPE) | (1<<MSTR) | (1<<CPOL) | (1<<CPHA) | (1<<SPR0));
    SPSR = (1<<SPI2X);
  #elif F_CPU < 16000000L
    // < 16MHz ... SCK frequency = oscillator frequency / 16
    SPCR = ((1<<SPE) | (1<<MSTR) | (1<<CPOL) | (1<<CPHA) | (1<<SPR0));
  #else
    // >= 16MHz ... SCK frequency = oscillator frequency / 32
